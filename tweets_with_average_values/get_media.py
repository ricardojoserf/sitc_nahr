...
with open(csvfile, 'r') as fin, open('results/'+csvfile, 'w') as fout:
	...
	for row in reader:
		if count_row == 0:
			writer.writerow([row[0]] + [row[1]] + [row[3]] + [row[4]] + [row[5]] + [row[6]] + [row[7]] + [row[8]] + [row[2]] +['media_tri'] + ['media_bin'] + ['total'])
			count_row+=1
		else:
			column=1
			total_pol = 0.0
			...
			writer.writerow([row[0]] + [row[1]] + [row[3]] + [row[4]] + [row[5]] + [row[6]] + [row[7]] + [row[8]] + [row[2]] +[str(tri_val)] + [str(bin_val)] + [str(total)])